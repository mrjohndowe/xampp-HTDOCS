...
try {
    ...
    $prompt = 'Analyze the provided video still frames together with the available local file context. Return ONLY a valid JSON object with exactly these fields: name (string), actors (array of strings), characters (array of strings), productions (array of strings), categories (array of strings), and summary (string). Output valid JSON only with no Markdown, code fences, commentary, explanations, or additional fields. Base all results only on information visible in the frames or explicitly provided by the file context. Be conservative when identifying people, characters, productions, games, locations, or other entities; never identify a real person by name unless their identity is clearly supplied by the filename or provided file context, and use empty arrays [] whenever actors, characters, productions, or categories cannot be determined confidently. Do not include explicit sexual detail; describe mature content only in neutral, non-graphic language. Keep the summary concise and describe only what is visibly happening without inventing identities, events, motivations, or unsupported context. The name MUST be an original, descriptive, human-readable library title of 4 to 10 words describing the actual visible moment and MUST NOT be only a game, franchise, production, character, person, filename, map, or mode name. NEVER use the game name by itself as the title, even when the game is confidently identified. First determine the most interesting visible action, objective, event, location, encounter, vehicle, weapon use, environmental moment, victory, failure, chase, fight, exploration, or other distinctive activity and build the title around that. For gameplay, ALWAYS begin with what is happening in the footage and optionally append the game name as secondary context; examples: "Holding the Objective Under Heavy Fire - Battlefield", "Last-Second Push Through the City - Battlefield", "Tank Assault Across the Burning Fields - Battlefield", or "Close-Quarters Fight Inside the Compound - Battlefield". Titles such as "Battlefield", "Battlefield 6", "Call of Duty", "Fortnite", "Battlefield Gameplay", or "Game Name - Secure Zones" are INVALID. If no specific event can be determined, create the best descriptive title possible from the visible environment, activity, mode, equipment, or setting rather than falling back to the game name. Treat file context as supporting metadata, prefer visible evidence when describing what happens, and use file context only to resolve information it clearly supplies. File name: ' . $video['file'] . '. Original name: ' . $video['original_name'] . '. Creation-derived published date: ' . ($video['publish_date'] ?: 'unknown') . '.';
