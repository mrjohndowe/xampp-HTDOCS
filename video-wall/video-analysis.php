...
$titleStatement = db()->prepare(
    "SELECT COALESCE(NULLIF(TRIM(display_name), ''), TRIM(original_name)) AS title
     FROM videos
     WHERE COALESCE(NULLIF(TRIM(display_name), ''), TRIM(original_name)) <> ''"
);
...
removeStaleAnalysisFrames($temporary);
if (
    ! is_dir($temporary)
...
$frames     = [];
$cleanupRegistered = register_shutdown_function(static function () use (&$frames): void {
...
try {
    ...
    */
    ...
    for ($attempt = 1; $attempt <= $maxAttempts; $attempt++) {
        ...
        $attemptSuggestion = null;
        foreach ($candidateTexts as $candidateText) {
            $candidateText = html_entity_decode($candidateText, ENT_QUOTES | ENT_HTML5, 'UTF-8');
            $candidateText = (string) preg_replace('/<think>.*?<\/think>/is', '', $candidateText);
            $candidateText = (string) preg_replace('/^```(?:json)?\s*|\s*```$/i', '', trim($candidateText));
            $decoded = json_decode($candidateText, true);
            if (!is_array($decoded)) {
                $start = strpos($candidateText, '{');
                $end = strrpos($candidateText, '}');
                if ($start !== false && $end !== false && $end > $start) {
                    $decoded = json_decode(substr($candidateText, $start, $end - $start + 1), true);
                }
            }
            if (is_array($decoded)) { $attemptSuggestion = $decoded; break; }
        }

        if (! is_array($attemptSuggestion)) {
            $duplicateTitle = 'The previous response was not valid JSON';
            if ($attempt === $maxAttempts) {
                throw new RuntimeException('Ollama returned an invalid JSON analysis after ' . $maxAttempts . ' attempts.');
            }
            continue;
        }
        ...
        if (in_array($normalizedTitle, $normalizedExistingTitles, true)) {
            ...
            if ($attempt === $maxAttempts) {
                // Do not fail an otherwise valid analysis just because the small
                // local model keeps returning the same title. Choose a genuinely
                // different human-readable hook instead of adding an identifier.
                $title = makeDistinctAnalysisTitle($title, $existingTitles);
    ...
    $suggestion['summary'] = (string) preg_replace(
        '/^(?:the|this)\s+(?:video|clip)\s+(?:depicts|shows|features)\s+/i',
        '',
        $suggestion['summary']
    );
    if ($suggestion['summary'] !== '') {
        $suggestion['summary'] = ucfirst($suggestion['summary']);
    }
