...
try {
    ...
    $prompt = 'Analyze the provided video still frames together with the available local file context. Return ONLY a valid JSON object with exactly these fields: name (string), actors (array of strings), characters (array of strings), studios (array of strings), productions (array of strings), categories (array of strings), genres (array of strings), and summary (string). Output valid JSON only with no Markdown, code fences, commentary, explanations, or additional fields. Base the analysis primarily on information visible in the frames while using the filename, original name, date, and other supplied context as supporting evidence. Never identify a real person by name unless their identity is clearly supplied by the filename or provided context, but you MAY intelligently suggest likely fictional characters, studios, developers, publishers, productions, franchises, categories, and genres when the available visual or file evidence reasonably supports the inference. Use [] only when there is not enough evidence to make a useful suggestion. For gameplay, studios may contain the likely developer or publisher, characters may contain recognizable named playable or story characters, productions may contain the game or franchise, categories should describe useful subjects or activities such as Gaming, FPS, Multiplayer, Objective Play, Combat, Vehicles, Airstrike, or Clutch, and genres should describe broader genres such as First-Person Shooter, Action, Battle Royale, Survival, Racing, Simulation, or Strategy. Do not include explicit sexual detail; describe mature content only in neutral, non-graphic language. Keep the summary concise and describe the apparent visible event without inventing unsupported specifics. The name MUST be a catchy, natural, curiosity-driven title or headline describing the most interesting moment in the footage, similar to a strong TikTok, YouTube Short, or gaming-highlight hook, and should create curiosity without being misleading. Focus on the action, surprise, tension, close call, objective, encounter, mistake, clutch, unexpected event, or memorable moment rather than simply identifying the content. Titles may use natural first-person phrasing when appropriate, such as "I Had No Idea What Was About to Hit This Objective", "Everything Hit the Objective at Once", "I Thought This Objective Was Already Lost", "This Push Got Out of Control Fast", or "I Was Not Ready for What Came Next". NEVER use only a game, franchise, production, studio, character, person, filename, map, or mode name as the title and NEVER simply clean up or copy the filename. For gameplay, the game name should normally be placed in productions rather than used as the title; if included in the title, it must only provide secondary context after the actual hook. Determine what is visibly happening first and build the title around the strongest moment. If the exact event is unclear, create a compelling but truthful title from the visible action, environment, objective, equipment, encounter, or setting instead of falling back to the game name. File name: ' . $video['file'] . '. Original name: ' . $video['original_name'] . '. Creation-derived published date: ' . ($video['publish_date'] ?: 'unknown') . '.';
