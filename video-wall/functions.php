...
function migrateLegacyJson(PDO $pdo): void
{
    ...
    try {
        ...
        $setting->execute(['debugEnabled', '0']);
        $pdo->commit();
...
{
    $pdo = db();
    $settings = ['folders' => [], 'ffmpegPath' => '', 'autoNext' => true, 'startMuted' => false, 'debugEnabled' => false];
    $settings['folders'] = $pdo->query('SELECT path FROM folders ORDER BY path COLLATE NOCASE')->fetchAll(PDO::FETCH_COLUMN);
    foreach ($pdo->query('SELECT key,value FROM settings') as $row) $settings[$row['key']] = in_array($row['key'], ['autoNext', 'startMuted', 'debugEnabled'], true) ? $row['value'] === '1' : $row['value'];
    return $settings;
}
...
function saveSettings(array $settings): void
{
    ...
    try {
        ...
        $statement->execute(['debugEnabled', !empty($settings['debugEnabled']) ? '1' : '0']);
        $pdo->commit();
    } catch (Throwable $error) {
        $pdo->rollBack();
        throw $error;
...
function makeDistinctAnalysisTitle(string $title, array $existingTitles): string
{
    $title = trim($title);
    $candidates = [
        'The Moment ' . $title . ' Took an Unexpected Turn',
        'I Was Not Ready for ' . $title,
        'Everything Changed During ' . $title,
        'The Close Call Inside ' . $title,
        'What Happened Next in ' . $title,
        'The Unexpected Finish to ' . $title,
        'This Part of ' . $title . ' Got Intense',
        'A Twist Nobody Saw Coming in ' . $title,
        'The Detail That Changed ' . $title,
    ];
    $used = [];
    foreach ($existingTitles as $existing) $used[normalizeVideoTitle((string)$existing)] = true;
    foreach ($candidates as $candidate) {
        if ($candidate !== '' && !isset($used[normalizeVideoTitle($candidate)])) return $candidate;
    }
    $fallbacks = ['A Moment Worth Watching', 'The Scene Took a Sudden Turn', 'This Moment Was Not Expected', 'The Most Unexpected Part'];
    foreach ($fallbacks as $fallback) if (!isset($used[normalizeVideoTitle($fallback)])) return $fallback;
    return 'A New Moment in the Footage';
}
...
function removeStaleAnalysisFrames(string $directory, int $maxAgeSeconds = 3600): void
{
