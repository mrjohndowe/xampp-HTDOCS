...
function db(): PDO
{
    ...
    $pdo->exec('PRAGMA journal_mode=WAL; PRAGMA foreign_keys=ON; CREATE TABLE IF NOT EXISTS settings (key TEXT PRIMARY KEY,value TEXT NOT NULL); CREATE TABLE IF NOT EXISTS folders (id INTEGER PRIMARY KEY AUTOINCREMENT,path TEXT NOT NULL UNIQUE); CREATE TABLE IF NOT EXISTS categories (id INTEGER PRIMARY KEY AUTOINCREMENT,name TEXT NOT NULL UNIQUE COLLATE NOCASE); CREATE TABLE IF NOT EXISTS production (id INTEGER PRIMARY KEY AUTOINCREMENT,name TEXT NOT NULL UNIQUE COLLATE NOCASE); CREATE TABLE IF NOT EXISTS videos (id TEXT PRIMARY KEY,original_name TEXT NOT NULL,display_name TEXT,file TEXT NOT NULL,category TEXT NOT NULL DEFAULT "",folder TEXT NOT NULL,path TEXT NOT NULL UNIQUE,extension TEXT NOT NULL,size INTEGER NOT NULL,modified INTEGER NOT NULL,category_id INTEGER REFERENCES categories(id) ON DELETE SET NULL,active INTEGER NOT NULL DEFAULT 1,actors TEXT NOT NULL DEFAULT "",characters TEXT NOT NULL DEFAULT "",publish_date TEXT NOT NULL DEFAULT "",production TEXT NOT NULL DEFAULT "",duplicate_of TEXT); CREATE TABLE IF NOT EXISTS video_categories (video_id TEXT NOT NULL REFERENCES videos(id) ON DELETE CASCADE,category_id INTEGER NOT NULL REFERENCES categories(id) ON DELETE CASCADE,PRIMARY KEY(video_id,category_id)); CREATE TABLE IF NOT EXISTS video_productions (video_id TEXT NOT NULL REFERENCES videos(id) ON DELETE CASCADE,production_id INTEGER NOT NULL REFERENCES production(id) ON DELETE CASCADE,PRIMARY KEY(video_id,production_id)); CREATE INDEX IF NOT EXISTS video_categories_category_idx ON video_categories(category_id); CREATE INDEX IF NOT EXISTS video_productions_production_idx ON video_productions(production_id)');
...
function buildCatalog(array $folders): array
{
    ...
    foreach ($folders as $root) {
        try {
            ...
            foreach ($iterator as $file) {
                ...
                $id = hash('sha256', $path);
                $items[] = ['id' => $id, 'original_name' => pathinfo($file->getFilename(), PATHINFO_FILENAME), 'display_name' => $names[$id] ?? null, 'file' => $file->getFilename(), 'category' => '', 'folder' => basename($root) . ($relative && dirname($relative) !== '.' ? ' / ' . dirname($relative) : ''), 'path' => $path, 'extension' => $extension, 'size' => $file->getSize(), 'modified' => $file->getMTime(), 'category_id' => $assignments[$id] ?? null, 'active' => isset($activeStates[$id]) ? (int)$activeStates[$id] : 1, 'actors' => $metadata[$id]['actors'] ?? '', 'characters' => $metadata[$id]['characters'] ?? '', 'publish_date' => $metadata[$id]['publish_date'] ?? '', 'production' => $metadata[$id]['production'] ?? ''];
            }
    ...
    try {
        ...
        $statement = $pdo->prepare('INSERT INTO videos(id,original_name,display_name,file,category,folder,path,extension,size,modified,category_id,active,actors,characters,publish_date,production,duplicate_of) VALUES(:id,:original_name,:display_name,:file,:category,:folder,:path,:extension,:size,:modified,:category_id,:active,:actors,:characters,:publish_date,:production,:duplicate_of)');
